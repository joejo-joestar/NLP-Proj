...
from nlp_track_b.person1.config import ModelConfig, PipelineConfig
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for split in ("train", "val", "test"):
                ...
                for file_path in outputs_dir.glob("*.json"):
                    ...
                    self.assertIn("answer_start_token_idx", payload)
                    self.assertIn("answer_end_token_idx", payload)
                    self.assertIn("token_outputs", payload)
                    self.assertIn("token_alignment", payload)
                    self.assertEqual(payload["token_length"], len(payload["input_ids"]))
                    self.assertEqual(payload["token_length"], len(payload["attention_mask"]))
                    self.assertLess(
                        payload["answer_start_token_idx"],
                        payload["answer_end_token_idx"],
                    )
