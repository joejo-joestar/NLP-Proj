...
def _load_train_record(path: Path, *, require_logits: bool, expected_split: str) -> dict:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("input", type=Path, help="Train Person 1 artifact file or directory")
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    paths = _limited_paths(args.input, args.limit)
    expected_split = args.expected_split or ""

    def records(require_logits: bool = False):
        for path in paths:
            yield _load_train_record(path, require_logits=require_logits, expected_split=expected_split)
    ...
    if args.include_logit_lens:
        first_record = _load_train_record(paths[0], require_logits=True, expected_split=expected_split)
    ...
    for record in records(require_logits=args.include_logit_lens):
        ...
        pca = compute_pca_deviation(record["hidden_states"], answer_start, answer_end, pca_stats)
        metric_values["cosine_drift"].append(cosine["cosine_drift"])
        metric_values["mahalanobis_distance"].append(mahalanobis["mahalanobis_distance"])
        ...
        if model is not None:
            ...
            metric_values["logit_lens_divergence"].append(logit_lens["logit_lens_divergence"])
    ...
    print(f"layers_used={mahalanobis_stats['layers_used']}")
